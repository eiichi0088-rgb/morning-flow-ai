...
type LlmAssistantResult = {
  ...
  assistantLinesCount: number;
  calendarCount: number;
  conversationUnderstanding: ConversationUnderstanding | null;
  draftAutoGenerated: boolean;
  draftItemCount: number;
  draft: MorningReviewDraft;
  extractedCalendarCandidates: string[];
...
function App() {
  ...
  const [conversationUnderstanding, setConversationUnderstanding] = React.useState<ConversationUnderstanding | null>(null);
  ...
  const resetLocalWorkspaceState = React.useCallback(() => {
    ...
    setConversationUnderstanding(null);
  ...
  const processMorningConversationTurn = React.useCallback(
    async (text: string) => {
      ...
      try {
        ...
        nextConversationUnderstanding = llmResult.conversationUnderstanding;
...
async function processLlmAssistantTurn({
  ...
  if (isConversationSaveCommand(text)) {
    ...
    const conversationUnderstanding = hasDraft
      ? createConversationUnderstandingFromDraft(conversationDraft, assistantLines.join('\n'))
      : null;
    return {
...
function createConversationUnderstandingFromDraft(
  draft: MorningReviewDraft,
  fallbackSummary: string,
): ConversationUnderstanding {
  const calendarEvents = createCalendarEvents(draft.plan);
  const suggestedSaveTargets = {
    schedules: cleanScheduleItems(draft.plan.schedule).map((item) => [item.time, item.task].filter(Boolean).join(' ')),
    shopping: draft.shoppingItems.map(formatShoppingItemLabel),
    followUps: draft.followUpCandidates.map((item) => [item.name, item.content].filter(Boolean).join(' ')),
    calendarCandidates: calendarEvents.map((event) =>
      [event.start.toLocaleDateString('ja-JP'), formatScheduleDisplayTime(event.sourceTime, event.start), event.title].filter(Boolean).join(' '),
    ),
  };
  const summary = fallbackSummary.trim() || 'AIは保存前確認へ進む内容として理解しました。';
  return {
    summary,
    understoodItems: createUnderstoodItemsFromSummary(summary, suggestedSaveTargets),
    suggestedSaveTargets,
  };
}

function pureUnderstandingFallbackText(text: string) {
  return text.trim() ? `AIは「${text.trim()}」を整理対象として理解しました。` : 'AIは入力内容を整理対象として理解しました。';
}

function createUnderstoodItemsFromSummary(
  summary: string,
  suggestedSaveTargets: ConversationUnderstanding['suggestedSaveTargets'],
) {
  const summaryItems = summary
    .split(/[\n。.!！?？]/)
    .map((line) => line.replace(/^[\s・\-*]+/, '').trim())
    .filter((line) => line.length >= 2 && !/^AI[はが]/.test(line))
    .filter((line) => !/保存候補|Review Draft|保存前確認/.test(line));
  const targetItems = [
    ...suggestedSaveTargets.schedules,
    ...suggestedSaveTargets.shopping,
    ...suggestedSaveTargets.followUps,
    ...suggestedSaveTargets.calendarCandidates,
  ];
  return dedupeByKey([...summaryItems, ...targetItems], normalizeTaskText).slice(0, 12);
}

function createUnderstandingConfidenceNote(
  understoodItems: string[],
  suggestedSaveTargets: ConversationUnderstanding['suggestedSaveTargets'],
) {
  const targetCount =
    suggestedSaveTargets.schedules.length +
    suggestedSaveTargets.shopping.length +
    suggestedSaveTargets.followUps.length +
    suggestedSaveTargets.calendarCandidates.length;
  if (understoodItems.length > targetCount) {
    return 'AIの理解内容と保存候補は別枠です。抜けがある場合は下のReview Draftで確認してください。';
  }
  return '保存処理は既存のReview Draft候補だけを使います。';
}
...
function ConversationUnderstandingCard({
  ...
  return (
    <section className="conversation-understanding-card" aria-label="AI理解カード">
      <div className="conversation-understanding-header">
        <div>
          <span>Conversation First</span>
          <strong>AIはこう理解しました</strong>
        </div>
        <MessageCircle size={20} />
      </div>

      {understanding.summary && <p className="conversation-understanding-summary">{understanding.summary}</p>}

      <ul className="conversation-understanding-list">
        {visibleUnderstoodItems.map((item) => (
          <li key={item}>{item}</li>
        ))}
      </ul>

      <div className="conversation-save-targets" aria-label="保存候補">
        <span>保存候補</span>
        <div>
          <strong>予定</strong>
          <b>{scheduleCount}件</b>
        ...
        <div>
          <strong>カレンダー候補</strong>
          <b>{calendarCount}件</b>
        </div>
      </div>

      <div className="conversation-target-preview">
        <ConversationTargetPreview title="予定候補" items={understanding.suggestedSaveTargets.schedules} />
        <ConversationTargetPreview title="買い物候補" items={understanding.suggestedSaveTargets.shopping} />
...
function ConversationTargetPreview({ title, items }: { title: string; items: string[] }) {
  return (
    <div className="conversation-target-preview-group">
      <span>{title}</span>
      {items.length ? (
        <ul>
